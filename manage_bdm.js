...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function enterEditMode(index, isNew = false) {
    ...
    row.children[5].innerHTML = `<input type="date" value="${item.dueDate || ""}">`;
    ...
    editBtn.addEventListener("click", () => {
      ...
      const newDue = row.querySelector("input[type='date']").value;
    ...
    if (!isNew && (item.fileDisplay !== "" || item.fileName !== "")) unsavedEdits.add(index);
  }
